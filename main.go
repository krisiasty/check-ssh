...
// config maps lowercased sshd_config option names to one or more values.
// Most options have a single value; directives like `hostkey` may repeat.
...
func (r rule) check(c config, res *result) {
	vals, ok := c[strings.ToLower(r.option)]
	if !ok || len(vals) == 0 {
		slog.Error("missing required setting", "option", r.option)
		res.missing++
		return
	}
	enabled := vals[len(vals)-1] // last wins for repeated directives
...
func getSshdConfig(path string) ([]byte, error) {
	...
	if err != nil {
		...
		slog.Error("error executing sshd -T", "path", p, "stderr", strings.TrimSpace(stderr), "err", err.Error())
		if strings.Contains(stderr, "no hostkeys available") {
			slog.Warn("sshd has no host keys; run 'sudo ssh-keygen -A' to generate them (on macOS, enabling Remote Login in System Settings does the same)")
		}
...
func getRemoteConfig(host string, port int) (config, error) {
	...
	c := make(config)
	c["kexalgorithms"] = []string{filterKexExtensions(kexAlgos)}
	c["hostkeyalgorithms"] = []string{hostKeyAlgos}
	c["ciphers"] = []string{mergeAlgos(encCS, encSC)}
	c["macs"] = []string{mergeAlgos(macCS, macSC)}
	return c, nil
}
...
// readHostKeyPub parses an OpenSSH-format public key file and returns the key
// type and size in bits. RSA/DSA size is the bit length of the modulus / p;
// ECDSA size is the curve size; Ed25519/Ed448 sizes are fixed by the algorithm.
func readHostKeyPub(path string) (string, int, error) {
	data, err := os.ReadFile(path) // #nosec G304 -- path comes from sshd's own configuration
	if err != nil {
		return "", 0, err
	}
	fields := strings.Fields(string(data))
	if len(fields) < 2 {
		return "", 0, fmt.Errorf("malformed public key file")
	}
	blob, err := base64.StdEncoding.DecodeString(fields[1])
	if err != nil {
		return "", 0, fmt.Errorf("decoding base64: %w", err)
	}
	keyType, off, err := parseNameList(blob, 0)
	if err != nil {
		return "", 0, fmt.Errorf("reading key type: %w", err)
	}
	switch keyType {
	case "ssh-ed25519":
		return keyType, 256, nil
	case "ssh-ed448":
		return keyType, 456, nil
	case "ecdsa-sha2-nistp256":
		return keyType, 256, nil
	case "ecdsa-sha2-nistp384":
		return keyType, 384, nil
	case "ecdsa-sha2-nistp521":
		return keyType, 521, nil
	case "ssh-rsa":
		_, off, err = parseNameList(blob, off) // skip e (public exponent)
		if err != nil {
			return "", 0, fmt.Errorf("reading exponent: %w", err)
		}
		n, _, err := parseNameList(blob, off) // n (modulus)
		if err != nil {
			return "", 0, fmt.Errorf("reading modulus: %w", err)
		}
		return keyType, mpintBits([]byte(n)), nil
	case "ssh-dss":
		p, _, err := parseNameList(blob, off) // p (prime)
		if err != nil {
			return "", 0, fmt.Errorf("reading p: %w", err)
		}
		return keyType, mpintBits([]byte(p)), nil
	default:
		return keyType, 0, fmt.Errorf("unsupported key type: %s", keyType)
	}
}

// mpintBits returns the bit length of an SSH mpint, stripping the leading
// zero sign byte if present (RFC 4251 §5).
func mpintBits(b []byte) int {
	...
	return (len(b)-1)*8 + (8 - bits.LeadingZeros8(b[0]))
}

// checkHostKeySizes inspects each `hostkey` path's `.pub` file and applies
// per-keytype size policy. Only meaningful for RSA (variable size); ED25519,
// ED448, and ECDSA sizes are fixed by the algorithm name and are checked via
// the HostKeyAlgorithms rule. DSA is prohibited unconditionally.
func checkHostKeySizes(c config, res *result) {
	paths, ok := c["hostkey"]
	if !ok || len(paths) == 0 {
		slog.Warn("no host keys configured in sshd")
		res.warnings++
		return
	}
	for _, path := range paths {
		pubPath := path + ".pub"
		algo, keyBits, err := readHostKeyPub(pubPath)
		if err != nil {
			slog.Warn("cannot read host key", "path", pubPath, "err", err.Error())
			res.warnings++
			continue
		}
		slog.Info("host key", "path", pubPath, "algo", algo, "bits", keyBits)
		switch algo {
		case "ssh-rsa":
			switch {
			case keyBits < rsaMinAcceptable:
				slog.Error("RSA host key below acceptable size", "path", pubPath, "bits", keyBits, "minimum", rsaMinAcceptable)
				res.errors++
			case keyBits < rsaMinRecommended:
				slog.Warn("RSA host key below recommended size", "path", pubPath, "bits", keyBits, "recommended", rsaMinRecommended)
				res.warnings++
			}
		case "ssh-dss":
			slog.Error("DSA host key is prohibited", "path", pubPath)
			res.errors++
		}
		// Ed25519/Ed448/ECDSA: size is fixed by the algorithm; classification handled by HostKeyAlgorithms rule.
	}
}
...
func main() {
	...
	if p.host != "" {
		...
		slog.Warn("host key sizes cannot be verified in remote mode; use local mode for size checks")
	} else {
		var buf []byte
		var err error
		if p.config != "" {
